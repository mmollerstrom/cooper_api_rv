RSpec.describe Api::V1::PerformanceDataController, type: :request do
  ...
  describe 'POST /api/v1/performance_data' do
    it 'creates a data entry' do
      POST '/api/V1/performance_data', params: { performance_data: { data: { message: 'Average' } } }, headers: headers

      entry = PerformanceData.last
      expect(entry.data).to eq 'message' => 'Avarage'
    end
  end

  describe 'GET /api/v1/performance_data' do
    before do
      5.times { user.performance_data.create(data: { message: 'Avarage' }) }
    end
